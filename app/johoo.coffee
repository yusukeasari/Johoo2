...
class PhotomosaicViewer extends Backbone.View
  ...
  setup:(_init)=>
    ...
    #タイムラインクリック時のイベント
    @quickSearchPanel.on 'onclicksearchresult',(d) =>
      @router.navigate "quicksearch/#{d}/",
        trigger: true
...
#      @pyramid.pyramidSetPositionToCenter()
    ...
    @controlPanel.on 'onclickformbutton', =>
      $.fancybox.open
        ...
        touch: false
    #画像コンバート
    @quickSearchPanel.on 'convertimage', (_src)=>
      @ci.convertImage(_src,@quickSearchPanel.test,{crop:true,maxHeight:300,maxWidth:300})
...
class CI extends Backbone.View
  el: '#CI'
...
class QuickSearchPanel extends Backbone.View
  el: '#QuickSearch'
  menuopenbutton: '#menu-open'
  resultel: '#QuickSearchResult'
  textform: '#QuickSearchInput'
  searchbutton: '#QuickSearchButton'
...
#          'height':'200px'
    ...
    $('.searchTargetItemLoading').animate({opacity:0},0)
  ...
  getImageData:=>
    ...
    $(@resultel).css
      ...
      'width':'225px'
      'margin-top':'-335px'
    file = $(@fileform).prop('files')[0]
    ret = @checkFileType(file)
    if ret
      @trigger 'convertimage',ret
#      @setImageSize(ret)
    else
      return false
    $(@fileform).val('')
  ...
  displayOriginal:(_img)=>
    ...
    closebutton = 
    wrapper = $('<div >').attr('class','searchTargetItem').animate({opacity:0,marginTop:"+=20px"},0)
    wrapper.append img
    wrapper.append textarea
    $('.searchTargetItemLoading').animate({opacity:1},300,'easeOutQuart')
    $(@searchbutton).animate({opacity:0},300,'easeOutQuart')

#    wrapper.append $('<div >').attr('class','searchTargetItemLoading').animate({opacity:0},0)
  ...
  test:(_img)=>
    ...
    @displayOriginal(_img)
    $('.searchTargetItem').animate({opacity:1,marginTop:"0px"},300,'easeOutQuart', => @startImageSearch(_img))

  startImageSearch:(_img)=>
    imgSearchApi='image_search/rekognition_search_face_by_image2.php'
    $.ajax(
      type:"POST"
      url:imgSearchApi
      data:{image:_img.toDataURL('image/jpeg')}
      dataType:"json"
    ).done( (data)=>
      @displayResult(data)
    ).fail( (data)=>
  ...
  disableSearchButton:=>
    @i=0
    if faceRekognition
      $(@fileform).prop('disabled',true)
      $('#image').hide()
    $(@textform).prop('disabled',true)
    $(@searchbutton).off("click")
    $('.searchTargetItemLoading').show()

  enableSearchButton:=>
    $('.searchTargetItemLoading').animate({opacity:0},300,'easeOutQuart')
    $('.searchTargetItemLoading').hide()
    $(@searchbutton).animate({opacity:1},300,'easeOutQuart')
    if faceRekognition
      $(@fileform).prop('disabled',false)
      $('#image').hide()
    $(@textform).prop('disabled',false)
    $(@searchbutton).on 'click',@startSearch

  removeSearchResult:=>
    list = $(@resultel).find('div')
    list.map (n,_div)=>
      if _div != null && $(_div).attr("id") != undefined
        target = $(_div).attr("id")
        if $._data($(_div).get(0),"events") != undefined
          $(_div).off('click')
        if target.match(new RegExp('searchResultItem')) && !target.match(new RegExp('searchResultItem1'))
          $(_div).delay(n*20).animate({opacity:0,marginBottom:"+=50px"},300,'easeOutQuart')
          ...
          $(_div).delay(n*20).animate({opacity:0,marginBottom:"+=50px"},300,'easeOutQuart',@removeSearchResult)
    $(@resultel).empty()
    $(@resultel).css
      'height':'0px'
...
#        textarea = $('<div >').attr('class','searchResultItemText').html(((Math.floor(_img.score)-1000)/-10)+"%一致")
        wrapper = $('<div >').attr('id','searchResultItem'+i).attr('class','searchResultItem').animate({opacity:0,marginLeft:"+=50px"},0)
        wrapper.append img
        wrapper.append textarea
#        wrapper.animate({opacity:1,marginLeft:"0px"},500,'easeInQuart').delay(i*1000)
        $(@resultel).prepend(wrapper)
        h = ($('#searchResultItem'+i).outerHeight()+Math.floor($('#searchResultItem'+i).outerHeight()/8))*(_list.length-i)
        $('#searchResultItem'+i).css(marginTop:h+15+'px')
        $('#searchResultItem'+i).on('click',=>
          @onClickSearchResult(_img)
        )
        $('#searchResultItem'+i).delay(i*80).animate({opacity:1,marginLeft:"0px"},300,'easeOutQuart')
        #obj.animate({opacity:1},1000).delay(i*1000)
        i++
      @setResultElPos i+1
#    @enableSearchButton()
  ...
  startSearch:(e)=>
    ...
    $.ajax(
      ...
      if data[1][0].ERROR is 'TOOMUCHRESULT'
        @enableSearchButton()
        alert '300件以上ヒットしました。詳しく入力してください。'        
      if data[1][0].ERROR is 'NOTFOUND'
        @enableSearchButton()
        alert '入力された情報でヒットしませんでした。'        
      else
        @displayTextSearchResult(data)
    ).fail( (data)=>
      @enableSearchButton()
    )
    ...
    imgs.loadStart().done (_list)=>
      ...
      _list.map (_img)=>
        if @i <= @max
          ...
          h = ($('#searchResultItem'+@i).outerHeight()+Math.floor($('#searchResultItem'+@i).outerHeight()/8))*(m-@i)+l
          $('#searchResultItem'+@i).css(marginTop:h+'px')
          $('#searchResultItem'+@i).on('click',=>
            @onClickSearchResult(_img)
          )
...
class ImageLoader extends Backbone.View
  d:null
  target:[]
  list:[]
  options:{}
  ...
  loadItem:(_target)=>
    ...
    loadImage.parseMetaData _target.img, (_data)=>
      if _data.exif
        @option.orientation = _data.exif.get('Orientation')
#      loadImage(_target,@loadComplete,@options)
      loadImage(
        _target.img
        ,(_data)=>
          _target.data = _data
...
#    if Browser.device isnt 'pc'
      ...
      $(@el).on 'touchcancel',@onAndMouseCancel
    ...
    #背景を設定
    $('<img/>').attr('src',"#{bgImageApi}").on 'load',=>
      $(this).remove()    
      $(@el).css
  ...
  onAndMouseMove:(e)=>
    ...
    cords = Point.getPoint e
    if cords[0].length > 1
      disMv = @distance(cords[0],cords[1])
      @scale = (disMv/@disSt)

      localX = @dragStartX-@dragStartLeft
      localY = @dragStartY-@dragStartTop

      dx = (zoomSize[nowZoom][0]-(zoomSize[nowZoom][0]*@scale))/2
      dx = (dx/@scale)+(zoomSize[nowZoom][0]-localX)

      dy = (zoomSize[nowZoom][1]-(zoomSize[nowZoom][1]*@scale))/2
      dy = (dy/@scale)+(zoomSize[nowZoom][1]-localY)
    ...
    if Point.isLock() is false and @scale isnt -1
      ...
      if @scale > 1
        for item in zoomSize
          if zoomSize[nowZoom][0]*@scale > item[0] and item[0] isnt ""
          else if item[0] isnt undefined
            break
          cnt++
      else
        for item in zoomSize
          if zoomSize[nowZoom][0]*@scale < item[0]
            break
          cnt++
